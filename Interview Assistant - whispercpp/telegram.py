...
class TelegramSender:
    ...
    async def get_updates(self, offset: int | None = None, timeout: int = 20) -> list:
        params = {"timeout": timeout}
        if offset is not None:
            params["offset"] = offset
        try:
            resp = await self.client.get(f"{self.base}/getUpdates", params=params)
    ...
    async def download_file(self, file_path: str, dest) -> bool:
        url = f"https://api.telegram.org/file/bot{self.cfg.telegram_bot_token}/{file_path}"
        try:
