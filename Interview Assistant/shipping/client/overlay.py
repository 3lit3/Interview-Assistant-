...
class OverlayWindow:
    def __init__(self, bus: EventBus, on_mode_change=None, on_context_change=None,
                 on_start=None, on_end=None, on_telegram_settings=None):
        self.bus = bus
        self.on_mode_change = on_mode_change
        self.on_context_change = on_context_change
        self.on_start = on_start
        self.on_end = on_end
        self.on_telegram_settings = on_telegram_settings
    ...
    def _build(self):
        ...
        ttk.Label(top, textvariable=self.rec_var, foreground="gray",
                  font=("Segoe UI", 10, "bold")).pack(side="left", padx=10)
        self.start_btn = ttk.Button(top, text="▶ Start", command=self._start_pressed)
        self.start_btn.pack(side="left", padx=2)
        self.end_btn = ttk.Button(top, text="■ End", command=self._end_pressed,
                                  state="disabled")
        self.end_btn.pack(side="left", padx=2)
        ttk.Button(top, text="🔔 Telegram", command=self._telegram_pressed).pack(side="left", padx=6)
        ...
        ctx_btn_row = ttk.Frame(bot)
        ctx_btn_row.pack(fill="x", pady=(4, 0))
        ttk.Button(ctx_btn_row, text="Submit Context", command=self._context_submit).pack(side="right")
    ...
    def _telegram_pressed(self):
        if self.on_telegram_settings:
            self.on_telegram_settings()

    def set_recording(self, active: bool):
        self._recording = active
        self.start_btn.configure(state="disabled" if active else "normal")
        self.end_btn.configure(state="normal" if active else "disabled")
        self.rec_var.set("● Recording" if active else "○ Idle")

    def _context_changed(self, _evt=None):
        if self.on_context_change:
            self.on_context_change(self.context_box.get("1.0", "end").strip())

    def _context_submit(self):
        text = self.context_box.get("1.0", "end").strip()
        if self.on_context_change:
            self.on_context_change(text)
        self._note(f"Context submitted: {text[:80]}{'...' if len(text) > 80 else ''}")
