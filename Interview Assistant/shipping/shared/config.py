...
# In dev mode: .env is in client/ (one level up from shared/).
# In frozen exe: _bootstrap_frozen_env() loads .env from exe directory instead.
_ENV_SEARCH = [Path(__file__).resolve().parent.parent / "client" / ".env",
...
class Config:
    ...
    # LLM API key: set at runtime via key issuance (not from .env)
    llm_api_key: str = ""
...
_ENV_SKIP = {"TELEGRAM_BOT_TOKEN", "TELEGRAM_CHAT_ID"}
